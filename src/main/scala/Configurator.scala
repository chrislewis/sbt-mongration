...
object Configurator {
  /* 2.8 named params pls k thx */
  def apply(database: String): Configurator =
    Configurator("localhost", 27017, database, None)
    
  def apply(host: String, database: String): Configurator =
    Configurator(host, 27017, database, None)
}
